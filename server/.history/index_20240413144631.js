...
io.on("connection", (socket) => {
    ...
    socket.on("register", (data) => {
        ...
        db.query('Select * from user where email=? or nickname=? ', [data.email, data.uname], (err, result) => {
            if (!err) {
                if (result.length == 0) {
                    db.query('Insert into user (`fname`,`lname`,`nickname`,`email`,`pwd`,`hobby`,`interest`,`genre`) values (?,?,?,?,?,?,?,?)', [data.fname, data.lname, data.nname, data.email, encryptData(data.pwd), data.hobby, data.interest, data.genre], (err, result) => {
                        if (err) {
                            console.log(err);
                        }
                        else {
