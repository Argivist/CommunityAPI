...
io.on("connection", (socket) => {
    ...
    socket.on("get_rooms", async (data) => {
        ...
        try {
            ...
            const userRoomResult = await query(userRoomQuery, [users[data]]);
    
            const rooms = {
                rooms: [],
                recents: [], // You might want to revisit what this array should contain
                names: []
            };
    
            await Promise.all(userRoomResult.map(async (room) => {
                ...
                if (room.name !== null) {
                    ...
                    const usersInRoomResult = await query(usersInRoomQuery, [room.rid]);
    
                    await Promise.all(usersInRoomResult.map(async (user) => {
                        if (user.uid !== users[data]) {
                            const userQuery = 'SELECT * FROM user WHERE uid=?';
                            const userResult = await query(userQuery, [user.uid]);
