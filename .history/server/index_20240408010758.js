...
io.on("connection", (socket) => {
        ...
        db.query('Select * from message where rid=?', [data], (err, result) => {
            if (err) {
                ...
                result.forEach((message) => {
                    messages.push({ uid: Object.keys(users).find(key => users[key] === message.uid), rid: message.rid, mcontent: message.mcontent, mdate: message.mdate });
                    console.log("📬:Messages sent to " + users[message.uid]);
            })
        }
        })
    ...
    socket.on("send_message", (data) => {
        console.log("📩" + data.uid + " sent message to " + data.rid + " in room " + data.room);
        db.query('Insert into message (`uid`,`rid`,`mcontent`,`mdate`) values (?,?,?,?)', [users[data.uid], data.rid, data.message, new Date()], (err, result) => {
            if (err) {
                console.log(err);
            }
            else {
                console.log("📩" + users[data.uid] + " sent message to " + data.rid + " in room " + data.room);
            }
        });
