...
io.on("connection", (socket) => {
    ...
    socket.on("get_rooms", (data) => {
        ...
        db.query('SELECT * FROM userroom WHERE uid=?', [users[data]], async (err, result) => {
            if (err) {
                console.log(err);
            } else {
                ...
                for (const room of result) {
                    ...
                    if (room.name != null) {
                        ...
                        try {
                            const roomResult = await getUserRoom(room.rid,data);
                            if (roomResult.length > 0) {
                                const userResult = await getUser(roomResult[0].uid);
                                rooms.names.push(userResult.length > 0 ? userResult[0].nickname : "");
                            } else {
                                rooms.names.push("");
                            }
                        } catch (error) {
        ...
        async function getUserRoom(rid,t) {
            return new Promise((resolve, reject) => {
                db.query('SELECT * FROM userroom WHERE rid=? and uid!=?', [rid,data[t]], (err, result) => {
                    if (err) {
                        reject(err);
                    } else {
                        resolve(result);
            ...
            if (!err) {
                if (userRoomResult.length > 0) {
                    ...
                    db.query('INSERT INTO room (`rname`,`rdescription`) VALUES (?,?)', [data, "Friends"], (err, insertRoomResult) => {
                        console.log("🏠" + socket.id + " created room " + insertRoomResult.insertId);
                        if (!err) {
