...
io.on("connection", (socket) => {
    ...
    socket.on("find", (data) => {
        let users = [];
        let uid = [];
        db.query('SELECT * FROM user WHERE (nickname LIKE ? OR email LIKE ? OR fname LIKE ? OR lname LIKE ?) AND uid NOT ?', ['%' + data.val + '%', '%' + data.val + '%', '%' + data.val + '%', '%' + data.val + '%',users[data.token]], (err, result) => {
            if (err) {
                console.log(err);
            }
            else {
        ...
        db.query('SELECT message.*, user.nickname FROM message JOIN user ON message.uid = user.uid WHERE message.rid = ?', [data], (err, result) => {
            if (err) {
                ...
                result.forEach((message) => {
                    messages.push({ username: Object.keys(users).find(key => users[key] === message.uid), room: message.rid, message: message.mcontent, time: message.mdate, sender: message.nickname,message_type:message.mtype });
                });
    ...
    socket.on("send_message", (data) => {
        ...
        db.query('Insert into message (`uid`,`rid`,`mcontent`,`mdate`,`mtype`) values (?,?,?,?,?)', [users[data.username], data.room, data.message, new Date(),data.message_type], (err, result) => {
            if (err) {
        ...
        db.query('Select * from user where uid=?', [users[data.username]], (err, result) => {
            if (!err) {
                console.log(result);
                let d = { username: data.username, room: data.room, message: data.message, time: new Date(), sender: result[0].nickname ,message_type:data.message_type};
