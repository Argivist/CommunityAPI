...
io.on("connection", (socket) => {
    ...
    const db = require('./your_database_module'); // Import your database module
    ...
    socket.on("get_rooms", async (data) => {
        ...
        try {
            ...
            const userRoomResult = await db.query(userRoomQuery, [users[data]]);
            ...
            for (const room of userRoomResult) {
                ...
                if (room.name !== null) {
                    ...
                    const usersInRoomResult = await db.query(usersInRoomQuery, [room.rid]);
    
                    for (const user of usersInRoomResult) {
                        if (user.uid !== users[data]) {
                            const userQuery = 'SELECT * FROM user WHERE uid=?';
                            const userResult = await db.query(userQuery, [user.uid]);
