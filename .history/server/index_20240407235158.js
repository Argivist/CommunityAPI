...
io.on("connection", (socket) => {
    ...
    socket.on("reconnect", (data) => {
        user[data]=null;
        user[socket.id]=user[data];
        socket.emit("reconnected",{value:socket.id});
        console.log("🔗:A user " + data + "reconnected" + socket.id);
    });
    //Login
            ...
            if (err) {
                ...
                if (result.length > 0) {
                    if (result[0].pwd == data.pwd) {
                        ...
                        console.log("📜: The uid "+result[0].uid+" has been recorded");
