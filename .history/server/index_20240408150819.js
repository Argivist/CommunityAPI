...
io.on("connection", (socket) => {
    ...
    socket.on("send_message", (data) => {
        ...
        db.query('Insert into message (`xiauid`,`rid`,`mcontent`,`mdate`) values (?,?,?,?)', [users[data.username], data.room, data.message, new Date()], (err, result) => {
            if (err) {
                console.log(err);
            }
            else {
        ...
        db.query('Insert into room (`rname`,`rdescription`) values (?,?)', [data.name, data.description], (err, result) => {
            if(!err){
                db.query('Insert into Group_(`rid`,`gname`,`gdescription`) values (?,?,?)', [result.insertId, data.name, data.description], (err, result) => {
                    if(!err){
                        db.query('Insert into userroom (`uid`,`rid`) values (?,?)', [users[data.token], result.insertId], (err, result) => {
                            if(!err){
                                data.members.forEach((member) => {
                                    db.query('Insert into userroom (`uid`,`rid`) values (?,?)', [member, result.insertId], (err, result) => {
                                        if(!err){
    ...
    socket.on("find_group", (data) => {
        let groups = [];
        let gid = [];
        db.query('Select * from room where rname like ?', ['%' + data + '%'], (err, result) => {
            if (err) {
                console.log(err);
            }
            else {
