            ...
            if (err) {
                ...
                result.forEach((room) => {
                    ...
                    rooms.names.push(room.name);
                        ...
                        db.query('Select * from userroom where rid=? and uid !=?`', [room.rid,users[data]], (err, result) => {
                            db.query('Select * from user where uid=?', [result[0].uid], (err, result) => {
                            rooms.names.push(result[0].nickname);
                            });                         
                        });
                    }
                });
