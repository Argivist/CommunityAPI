...
io.on("connection", (socket) => {
    ...
    socket.on("get_messages", (data) => {
        ...
        db.query('SELECT message.*, user.username FROM message JOIN user ON message.uid = user.uid WHERE message.rid = ?', [data], (err, result) => {
            if (err) {
                console.log(err);
            }
            else {
                result.forEach((message) => {
                    messages.push({ username: Object.keys(users).find(key => users[key] === message.uid), room: message.rid, message: message.mcontent, time: message.mdate,sender:message.username });
                });
