    ...
    socket.on("create_room_with", (data) => {
        db.query('SELECT COUNT(*) AS num_rows FROM userroom WHERE uid IN (?,?) GROUP BY rid HAVING COUNT(DISTINCT uid) = 2;', [users[socket.id], data], (err, result) => {
            if(err){
                ...
                console.log("Result of first query:", result); // Log result array
                if(result.length > 0){
                    socket.emit("room_status", {status: "exists", value: result[0].rid});
                }else{
                    db.query('INSERT INTO room (`rname`,`rdescription`) VALUES (?,?)', [data, "Friends"], (err, result) => {
                        if(err){
                            console.log(err);
                        }else{
                            console.log("Result of second query:", result); // Log result array
                            db.query('SELECT * FROM room WHERE rname=?', [data], (err, result) => {
                                if(err){
                                    console.log(err);
                                }else{
                                    console.log("Result of third query:", result); // Log result array
                                    db.query('INSERT INTO userroom (`uid`,`rid`) VALUES (?,?)', [users[socket.id], result[0].rid], (err, result) => {
                                        if(err){
                                            console.log(err);
                                        }else{
                                            console.log("Result of fourth query:", result); // Log result array
                                            db.query('INSERT INTO userroom (`uid`,`rid`) VALUES (?,?)', [data, result[0].rid], (err, result) => {
                                                if(err){
                                                    console.log(err);
                                                }else{
                                                    console.log("Result of fifth query:", result); // Log result array
                                                    socket.emit("room_status", {status: "created", value: result[0].rid});
                                                    console.log("🏠" + socket.id + " created room " + result[0].rid);
                                                }
                                            });
                                        }
                                    });
