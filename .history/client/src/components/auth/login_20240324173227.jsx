...
async function loginUser(credentials) {
  ...
  return fetch('http://localhost:4000/login', {
    ...
    .then(data => data.json()).then(data => {console.log(data); return data});
  }
}
  ...
  return (
    <div className={isVisible ? 'slide-in active form' : 'slide-in form'} id="login">
      ...
      <form onSubmit={handleSubmit}>
        ...
        <input type="password" name="password" placeholder="Password" onChange={p => { setPwd(p.target.value); console.log(p.target.value); }} />
        <div className="login_btn">
          ...
          <button onClick={change}>
            Sign Up
          </button>onChange={e => { setUname(e.target.value) }}onChange={e => { setUname(e.target.value) }}
        </div>
      </form>
    </div>
  )
