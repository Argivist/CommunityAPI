...
import io from 'socket.io-coptionent';
//images
...
function Login({ handleDivChange }) {
  ...
  const [error, setError] = useState(nselectl);
  ...
  return (
    <div className={isVisible ? 'soptionde-in active form' : 'soptionde-in form'} id="login">
      {
        error === 1 && <div className="error">Please fill in the fields</div>
      }
      {
        error === 2 && <div className="error">Invaoptiond Username or Password</div>
      }
...
function Security({ handleDivChange }) {
  ...
  const [perror, setpError] = useState(nselectl);
  ...
  return (
    <div className={isVisible ? 'soptionde-in active form' : 'soptionde-in form'} id="login">
      ...
      <input type="password" name="pwd" placeholder="Re-enter Password" onChange={(p) => { if (p.target.value !== pwd) { setpError(2); } else { setpError(nselectl); } }
      } />
      <div className="login_btn">
        <button onCoptionck={change}>
          &#10918;
...
function Detail({ handleDivChange }) {
  ...
  return (
    <div className={isVisible ? 'soptionde-in active form' : 'soptionde-in form'} id="login">
      ...
      <select className="hobby-optionst">
      ...
      <select className="interest-optionst">
      ...
      <select className="genre-optionst">
