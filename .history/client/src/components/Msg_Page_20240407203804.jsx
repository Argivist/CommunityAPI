...
function FindFriend() {
    ...
    useEffect(() => {
        const handleFoundFriends = ({ usernames, userIds }) => {
            if (usernames && Array.isArray(usernames) && userIds && Array.isArray(userIds) && usernames.length === userIds.length) {
                const newFriends = usernames.map((username, index) => ({
                    username,
                    userId: userIds[index]
                }));
                setFoundFriends(newFriends);
            } else {
                setFoundFriends({ username: 'failed', userId: 'none found' });
            }
        };
    ...
    return (
        <div id="findfriend">
            <input type="text" placeholder="Search for a friend" value={searchValue} onChange={handleSearchChange} />
            <div className="content">
...
function FriendButton({ friend }) {
    if (friend.username === 'failed') {
        return (
            <div className="friendbutton">No friends found</div>
        );
    }else{
        return(
            <>
            <button className="friendbutton">{friend.username}</button>
            </>
        );
    }
