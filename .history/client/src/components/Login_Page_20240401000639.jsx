...
const UserProvider = ({ children }) => {
  ...
  return (
    <userContext.Provider value={{ user, setUser, pwd, setPwd, fname, setFname, lname, setLname, nname, setNname, email, setEmail, hobby, setHobby, interest, setInterest, genre, setGenre }}>
      {children}
...
function Login({ handleDivChange }) {
  ...
  const handleSubmit = async e => {
    ...
    try {
      socket.emit('login', { uname, pwd }, (data) => {
...
function RegName({ handleDivChange }) {
  ...
  const change = (opt) => {
    ...
    return (
      <div className={isVisible ? 'slide-in active form' : 'slide-in form'} id="login">
        <div className="login_btn">
          <button onClick={() => change(1)}>Log In</button>
