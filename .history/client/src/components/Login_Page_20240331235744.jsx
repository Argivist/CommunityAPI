...
function Login({ handleDivChange }) {
  ...
  const handleSubmit = async e => {
    ...
    const token = async () => {
      try{
        await socket.emit('login', { uname, pwd });
        socket.on('auth', (data) => {
          console.log(data);
        });
      }
      ...
      // try {
      //     const response = await socket.emit('login', { uname, pwd });
      //     const { token } = response; // Assuming the token is returned in the response object
      //     if (token) {
