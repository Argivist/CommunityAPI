...
// Function to set message list for a specific room
const setMessageListForRoom = (roomId, messages) => {
    setMessageLists(prevLists => ({
        ...prevLists,
        [roomId]: messages
    }));
};
    return (
        <MsgRoomContext.Provider value={{ Room, setRoom, joinRoom, leaveRoom, setMessageListForRoom, messageList, setMessageList }}>
            {children}
            ...
            <div className="content">
                {
                    rooms.map((room, index) => (
                        <Chate key={index} id={room} pic={icon} name={room} recent={recents[index]} />
                    ))
                }
            </div>
            <Toolbox />
...
const Chate = ({ id, pic, name, recent }) => {
    const { setRoom, joinRoom,leaveRoom,setMessageList } = useMsgRoom();
...
const Message_Section = () => {
    const { messageLists, setMessageListForRoom } = useMsgRoom();
    const { Room } = useMsgRoom();
    const messages = messageLists[Room] ;

    const targRef = useRef();

    useEffect(() => {
        socket.on("receive_message", (data) => {
            setMessageListForRoom(data.room, (prevMessages) => [...prevMessages, data]);
            targRef.current?.scrollIntoView({ behavior: "smooth" });
        });
        return () => {
            socket.off("receive_message");
        };
    }, [setMessageListForRoom]);
