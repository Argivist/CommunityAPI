...
const ChatList = () => {
    const {rooms,setRooms} = useMsgRoom();
                    ...
                    <div className="friendlist">
                        ...
                        {foundFriends.map((friend, index) => {
                            if (friend.username !== 'failed') {
                                return <FriendButton key={index} friend={friend.username} />
                            } else {
...
function FriendButton({ friend }) {
    const  {Room, setRoom, setRoomName,rooms,setRooms, joinRoom, leaveRoom,setOverlap} = useMsgRoom();
    return (
        <>
            <button className="friendbutton" onClick={
                ()=>{
                    leaveRoom(Room);
                    //creating the room
                    socket.emit("create_room_with", friend);
                    socket.on("room_status", (data) => {
                        if (data.status === "exists") {
                            ...
                            setRooms((rooms) => [...rooms, data.value]);
                            setRoom(data.value);
                            setRoomName(friend);
                            joinRoom(data.value);
                            document.getElementById('ChatList').className = "chatlist chatselected";
